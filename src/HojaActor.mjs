export default class HojaActor extends ActorSheet {
	...
	_getItemContextOptions() {
		...
		return [
			{
				name: game.i18n.localize("ARGS.sheet.general.item_edit.title"),
			...
			{
				name: game.i18n.localize("ARGS.sheet.general.item_delete.title"),
	...
	_onItemDelete(itemId) {
		...
		renderTemplate(
			"systems/ARGS/templates/dialog/delete-item.hbs",
			{name: itemData.name}
		).then(html => {
			new Dialog({
				...
				buttons: {
					Yes: {
						...
						label: `${game.i18n.localize("ARGS.dialog.general.yes")}`,
						callback: async () => {
					...
					Cancel: {
						...
						label: `${game.i18n.localize("ARGS.dialog.general.cancel")}`,
					},
				},
				default: "Yes",
			}).render(true);
